...
{
    public partial class ApproveRejectCHRequest : System.Web.UI.Page
    {
        ...
        private void ApproveRequest(int requestID)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                using (SqlCommand approveRequestCmd = new SqlCommand("Procedures_AdvisorApproveRejectCHRequest", conn))
                {
                    approveRequestCmd.CommandType = CommandType.StoredProcedure;
                    approveRequestCmd.Parameters.AddWithValue("@requestID", requestID);
                    approveRequestCmd.Parameters.AddWithValue("@current_sem_code", currentSemesterCode); 
        ...
        private void RejectRequest(int requestID)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                using (SqlCommand rejectRequestCmd = new SqlCommand("Procedures_AdvisorApproveRejectCHRequest", conn))
                {
                    rejectRequestCmd.CommandType = CommandType.StoredProcedure;
                    rejectRequestCmd.Parameters.AddWithValue("@requestID", requestID);
                    rejectRequestCmd.Parameters.AddWithValue("@current_sem_code", currentSemesterCode); 
