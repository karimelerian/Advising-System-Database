        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connstr))
                {
                    ...
                    string query = "SELECT name, course_id FROM dbo.FN_SemsterAvailableCourses(@semstercode)";
                    using (SqlCommand availcourses = new SqlCommand(query, conn))
                    {
                        String cursem = smstrcodetxt.Text;
                        ...
                        {
                            availcourses.Parameters.AddWithValue("@semstercode", cursem);
                        }
                        using (SqlDataReader rdr = availcourses.ExecuteReader(CommandBehavior.CloseConnection))
                        {
